...
const UnicornBackground: React.FC<UnicornBackgroundProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-0 w-full h-full bg-[#F2EFE7] overflow-hidden pointer-events-none">
      ...
      <div 
        className={`absolute bottom-0 w-full px-6 pb-12 pt-12 md:pb-16 bg-gradient-to-t from-[#F2EFE7] via-[#F2EFE7]/80 to-transparent transition-opacity duration-300 ${playerOpacity <= 0.05 ? 'pointer-events-none' : 'pointer-events-auto'}`}
      ...
      >
        <div className="max-w-xl mx-auto w-full flex flex-col gap-6 md:gap-8">
            ...
            <div className="flex flex-col items-center justify-center px-1 gap-1">
                 <span className="text-sm font-bold tracking-widest text-[#1A1A1A]/60 uppercase">
                    26.02.14 SAT | 16:00
                 </span>
                 <span className="text-sm font-bold tracking-widest text-[#1A1A1A]/60 uppercase">
                    赤玉春 VOL.2
            ...
            <div className="flex flex-col items-center justify-center pt-4 w-full gap-3">
                ...
                <button 
                    onClick={onMoreClick}
                    className="w-full py-4 bg-white/40 backdrop-blur-md border border-black/5 text-[#1A1A1A] text-sm font-bold rounded-xl hover:bg-white/60 transition-all flex items-center justify-center gap-2 shadow-lg active:scale-[0.98]"
                >
                    공연 정보 확인하기
                </button>
      ...
      <div 
        className="absolute inset-0 bg-white transition-opacity duration-100 ease-linear pointer-events-none"
        style={{
