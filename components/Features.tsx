  ...
  return (
    <div className="w-full py-24 px-6 md:px-12 lg:px-24 relative z-10 pointer-events-auto">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div id="reserve-card" className="order-1 md:order-2 group relative bg-white/60 backdrop-blur-md border border-black/5 rounded-[40px] p-10 overflow-hidden hover:border-black/20 transition-all duration-500 min-h-[640px] flex flex-col shadow-xl">
          ...
          <div className="relative z-10 mb-4 flex justify-between items-start min-h-[48px]">
            <div className="flex-1 pr-4">
                <h3 className="text-2xl font-bold text-[#1A1A1A] mb-1">
                    {reserveStage === 'intro' ? '티켓 예매' : 
                     reserveStage === 'form' ? '정보 입력' :
                     reserveStage === 'countSelection' ? '예매 매수 선택' :
                     reserveStage === 'confirm' ? '최종 정보 확인' :
                     reserveStage === 'success' ? '접수 완료' :
                     (reserveStage === 'finalGuide' || reserveStage === 'duplicateError') ? '안내사항' :
                     reserveStage === 'lookup' ? '예매 조회' : 
                     reserveStage === 'checkLoading' ? '조회 중' : '티켓 현황'}
                </h3>
                <p className={`text-sm font-medium transition-colors duration-300 ${reserveStage === 'success' ? 'text-gray-500' : 'text-gray-400'}`}>
                    {reserveStage === 'intro' ? '2026.02.14 SAT | 적옥춘 VOL.2' :
                     reserveStage === 'form' ? '정확한 한글 성함과 번호 4자리를 입력해주세요.' :
                     reserveStage === 'countSelection' ? '예매하실 티켓 수량을 선택해주세요.' :
                     reserveStage === 'confirm' ? '정보 수정이 불가합니다.' :
                     reserveStage === 'success' ? '계좌이체 이후 안내사항 숙지를 눌러주세요' :
                     (reserveStage === 'finalGuide' || reserveStage === 'duplicateError') ? '반드시 아래 내용을 확인해주세요.' :
            ...
            {(reserveStage === 'form' || reserveStage === 'countSelection' || reserveStage === 'lookup' || reserveStage === 'result') && (
                <button 
                  onClick={handleBack}
                  className="w-10 h-10 rounded-full bg-black/5 flex items-center justify-center hover:bg-black/10 transition-colors shrink-0"
                >
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'intro' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             ...
             <div className="mt-8 w-full space-y-3">
                <button 
                    ...
                    className="w-full py-4 bg-white text-[#1A1A1A] border border-[#1A1A1A]/10 text-sm font-bold rounded-xl hover:bg-gray-50 transition-all flex items-center justify-center gap-2 shadow-sm active:scale-[0.98]"
                >
                    티켓 발급받기
                </button>
                <button 
                    onClick={handleStartTicket}
                    className="w-full py-4 bg-[#1A1A1A] text-white text-sm font-bold rounded-xl hover:bg-[#1A1A1A]/90 transition-all flex items-center justify-center gap-2 shadow-lg active:scale-[0.98]"
                >
                    예매 시작하기
                </button>
             </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'form' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none translate-x-10'}`}>
             <div className="flex flex-col gap-5 flex-1 overflow-y-auto custom-scrollbar pr-1 pb-4">
                <div className="space-y-2">
                    <label className="text-xs font-bold text-[#1A1A1A]/60 flex items-center gap-2"><User className="w-4 h-4" /> 입금자 성함</label>
                    <input type="text" placeholder="한글 성함 입력" value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} className={`w-full bg-white/80 border ${formData.name && !isNameValid ? 'border-red-400' : 'border-black/10'} rounded-2xl px-5 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-black/5 transition-all`} />
                    {formData.name && !isNameValid && (
                        <p className="text-xs text-red-500 font-medium pl-1 mt-1">숫자 및 특수기호는 불가합니다</p>
                    )}
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-bold text-[#1A1A1A]/60 flex items-center gap-2"><Phone className="w-4 h-4" /> 연락처 뒷번호 (4자리)</label>
                    <input type="tel" maxLength={4} placeholder="1234" value={formData.phone} onChange={(e) => handleInputChange('phone', e.target.value.replace(/[^0-9]/g, ''))} className="w-full bg-white/80 border border-black/10 rounded-2xl px-5 py-4 text-sm tracking-widest focus:outline-none focus:ring-2 focus:ring-black/5" />
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-bold text-[#1A1A1A]/60 flex items-center gap-2"><Users className="w-4 h-4" /> 누구의 지인인가요?</label>
                    <div className="relative">
                        <select value={formData.referral} onChange={(e) => handleInputChange('referral', e.target.value)} className="w-full bg-white/80 border border-black/10 rounded-2xl px-5 py-4 text-sm appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-black/5">
             ...
             <div className="mt-8 w-full">
                <button 
                    ...
                    className={`w-full py-4 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg ${isFormValid && !isSubmitting ? 'bg-[#1A1A1A] text-white hover:bg-[#1A1A1A]/90 active:scale-[0.98]' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
                >
                   다음단계
                </button>
             </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'countSelection' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none translate-x-10'}`}>
             <div className="flex flex-col gap-5 flex-1 justify-center">
                <div className="space-y-4">
                    <label className="text-sm font-bold text-[#1A1A1A]/60 flex items-center justify-center gap-2"><Ticket className="w-5 h-5" /> 예매 매수</label>
                    <div className="flex items-center justify-center gap-10 py-4">
                        <button onClick={() => handleInputChange('count', Math.max(1, formData.count - 1))} className="text-4xl font-light text-[#1A1A1A] hover:opacity-50 transition-opacity w-12 h-12 flex items-center justify-center">-</button>
                        <span className="font-black text-[#1A1A1A] text-5xl">{formData.count}매</span>
                        <button onClick={() => handleInputChange('count', Math.min(10, formData.count + 1))} className="text-4xl font-light text-[#1A1A1A] hover:opacity-50 transition-opacity w-12 h-12 flex items-center justify-center">+</button>
                    </div>
                    <div className="text-center">
                         <p className="text-lg font-bold text-[#1A1A1A]/60 mt-4">
             ...
             <div className="mt-8 w-full">
                <button 
                    ...
                    className={`w-full py-4 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg bg-[#1A1A1A] text-white hover:bg-[#1A1A1A]/90 active:scale-[0.98]`}
                >
                   {isSubmitting ? (
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'confirm' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col justify-center overflow-y-auto custom-scrollbar">
                <div className="text-center space-y-2 mb-8">
                    <h4 className="text-xl font-bold text-[#1A1A1A]">입력하신 정보가 맞습니까?</h4>
                    <p className="text-sm text-[#C85E53] font-medium leading-relaxed">관리자 승인 후에는 정보 수정이 불가합니다.</p>
                </div>
                <div className="bg-white/40 backdrop-blur-md border border-black/5 rounded-2xl p-6 space-y-4">
                    <div className="flex justify-between border-b border-black/5 pb-2">
                        <span className="text-xs text-gray-400">성함 / 뒷번호</span>
                        <span className="text-sm font-bold">{formData.name} / {formData.phone}</span>
                    </div>
                    <div className="flex justify-between border-b border-black/5 pb-2">
                        <span className="text-xs text-gray-400">지인 (카테고리)</span>
                        <span className="text-sm font-bold">{formData.referral}</span>
                    </div>
                    <div className="flex justify-between border-b border-black/5 pb-2">
                        <span className="text-xs text-gray-400">예매 수량</span>
                        <span className="text-sm font-bold">{formData.count}매</span>
                    </div>
             ...
             <div className="mt-8 w-full flex gap-3">
                <button 
                  ...
                  className="flex-1 py-4 bg-white border border-black/10 text-sm font-bold rounded-xl active:scale-[0.98] transition-all disabled:opacity-50 hover:bg-gray-50"
                >
                  수정
                </button>
                <button 
                  ...
                  disabled={isSubmitting} 
                  className={`flex-1 py-4 bg-[#1A1A1A] text-white text-sm font-bold rounded-xl flex items-center justify-center active:scale-[0.98] transition-all relative overflow-hidden shadow-lg hover:bg-[#1A1A1A]/90 ${!isSubmitting ? 'gap-2' : ''}`}
                >
                    {isSubmitting ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'success' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col justify-center overflow-y-auto custom-scrollbar">
                <div className="flex flex-col items-center py-4">
                    <h2 className="text-2xl font-bold text-[#1A1A1A] mb-1">{(formData.count * TICKET_PRICE).toLocaleString()}원</h2>
                    <p className="text-sm text-gray-500 font-medium">1매 10,000원 × {formData.count}매</p>
                </div>
                <div className="w-full border-t border-black/5 my-2"></div>
                <div className="flex flex-col items-center text-center py-5">
                    <p className="text-sm font-bold text-[#1A1A1A] mb-2">입금 시 성함+뒷번호 4자리 변경 필수!</p>
                ...
                <div className="flex items-center justify-between py-4 px-2">
                    <div className="space-y-1">
                        ...
                        <p className="text-xl font-bold font-mono text-[#1A1A1A] tracking-tighter mt-1">21490204079748</p>
                    ...
                    <button 
                        ...
                        className="w-12 h-12 rounded-full bg-white/40 backdrop-blur-md border border-black/5 shadow-lg flex items-center justify-center hover:bg-white/60 transition-all active:scale-95 group/copy"
                    >
                        {isCopied ? <Check className="w-5 h-5 text-green-500" /> : <Copy className="w-5 h-5 text-gray-400 group-hover/copy:text-black/60" />}
                    </button>
                </div>
             </div>
             <div className="mt-4 w-full">
                ...
                }} className="w-full py-4 bg-[#1A1A1A] text-white text-sm font-bold rounded-xl hover:bg-[#1A1A1A]/90 transition-all shadow-lg active:scale-[0.98]">
                    안내사항 숙지
                </button>
             </div>
          </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'finalGuide' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col justify-center items-center text-center overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-6">
                    <h3 className="text-xl font-bold text-[#1A1A1A]">계좌 입금이 완료되었나요?</h3>
                    <p className="text-sm text-[#1A1A1A]/70 font-medium leading-relaxed">
                        ※ 입금 이후 티켓 발급하기를 클릭하여<br/>
                        당일에 확인 가능한 모바일 티켓을 발급해주세요
                    </p>
                </div>

             </div>
             <div className="mt-8 w-full">
                <button onClick={() => setReserveStage('intro')} className="w-full py-4 bg-[#1A1A1A] text-white text-sm font-bold rounded-xl hover:bg-[#1A1A1A]/90 transition-all shadow-lg active:scale-[0.98]">
                    처음으로 돌아가기
                </button>
             </div>
          </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'duplicateError' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col items-center justify-center text-center overflow-y-auto custom-scrollbar">
                ...
                <div className="w-full mb-8 space-y-2 shrink-0">
                     ...
                     <p className="text-lg font-bold text-[#1A1A1A]">{formData.name} / {formData.phone} / {formData.count}매</p>
                </div>

                {/* Divider */}
                <div className="w-full border-t border-black/5 mb-8 shrink-0"></div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'lookup' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none translate-x-10'}`}>
             <div className="flex-1 flex flex-col justify-center gap-6">
                <div className="space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold text-[#1A1A1A]/70">예매자 성함</label>
                        <input type="text" placeholder="홍길동" value={lookupData.name} onChange={(e) => setLookupData(prev => ({...prev, name: e.target.value}))} className={`w-full bg-white/80 border rounded-xl px-4 py-4 text-sm focus:outline-none ${lookupData.name && !isLookupNameValid ? 'border-red-400' : 'border-black/10'}`} />
                        {lookupData.name && !isLookupNameValid && (
                             <p className="text-xs text-red-500 font-medium pl-1 mt-1">숫자 및 특수기호는 불가합니다</p>
                        )}
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold text-[#1A1A1A]/70">연락처 뒷번호 4자리</label>
                        <input type="tel" maxLength={4} placeholder="1234" value={lookupData.phone} onChange={(e) => setLookupData(prev => ({...prev, phone: e.target.value.replace(/[^0-9]/g, '')}))} className="w-full bg-white/80 border border-black/10 rounded-xl px-4 py-4 text-sm tracking-widest focus:outline-none" />
                    </div>
                </div>
             ...
             <div className="mt-8 w-full">
                <button onClick={handleLookup} className="w-full py-4 bg-[#1A1A1A] text-white text-sm font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 hover:bg-[#1A1A1A]/90 active:scale-[0.98]">
                    정보 확인하기 <ChevronRight className="w-4 h-4" />
                </button>
             </div>
          </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'checkLoading' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col items-center justify-center gap-10">
                <div className="flex flex-col items-center gap-2">
                     <p className="text-[10px] text-[#1A1A1A]/60 font-bold tracking-[0.2em] uppercase">2026.02.14 SAT | 적옥춘 VOL.2</p>
                     <h3 className="text-2xl font-black text-[#1A1A1A]">{lookupData.name} {lookupData.phone}</h3>
                </div>
                ...
                <div className="bg-gray-50 px-6 py-3 rounded-full">
                    <p className="text-xs font-bold text-gray-500 animate-pulse">데이터를 검색 중입니다</p>
                </div>
             </div>
          </div>
                ...
                {lookupResult?.status === 'success' ? (
                    <>
                        ...
                        <div className="text-center shrink-0">
                            <p className="text-xs text-gray-400 font-medium bg-gray-50 px-4 py-2 rounded-full">
                    ...
                    <div className="flex flex-col items-center gap-6 py-10 text-center">
                        <div className="w-48 h-48 flex items-center justify-center">
                            <img 
                                src="https://i.postimg.cc/XN5ygSN0/Gemini-Generated-Image-r4ovb1r4ovb1r4ov-removebg-preview.png" 
                                alt="Not Found" 
                                className="w-full h-full object-contain" 
                            />
                        </div>
                        <div className="space-y-1">
                            <h4 className="text-xl font-bold text-[#1A1A1A]">정보 없음</h4>
                            <p className="text-sm text-gray-500">입력 정보를 다시 확인해주세요.</p>
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col items-center gap-6 py-10 text-center">
                        ...
                        <div className="space-y-1">
                            <h4 className="text-xl font-bold text-[#1A1A1A]">입금 확인 중입니다</h4>
                            <p className="text-sm text-gray-500">입금 내역을 확인하고 있습니다. <br/>조금만 더 기다려 주세요!</p>
                        </div>
                    </div>
                )}
             </div>
        ...
        {/* Location Card */}
        <div id="location-card" className="order-2 md:order-1 group relative bg-white/60 backdrop-blur-md border border-black/5 rounded-[40px] p-10 overflow-hidden hover:border-black/20 transition-all duration-500 min-h-[500px] grid grid-cols-1 grid-rows-1 shadow-xl">
          {/* Map View */}
          <div className={`col-start-1 row-start-1 flex flex-col h-full w-full transition-all duration-500 ease-in-out ${showParking ? 'opacity-0 -translate-x-10 pointer-events-none' : 'opacity-100 translate-x-0 z-10'}`}>
              <div className="relative z-10 shrink-0">
                <h3 className="text-xl font-bold text-[#1A1A1A] mb-4">찾아오시는 길 - 펄스 라이브홀</h3>
                <p className="text-sm text-[#1A1A1A]/80 leading-relaxed w-full font-medium whitespace-pre-wrap">
                  {`도로명 주소 : 서울특별시 서초구 주흥길 12 B1\n지번 주소 : 서울특별시 서초구 반포동 741 B1\n강남 교보타워 사거리 인근,\n9호선 신논현역 1번출구 도보 350m`}
                </p>
              </div>
              <div className="flex-1 w-full mt-6 mb-6 rounded-3xl overflow-hidden relative border border-black/5 bg-gray-100 shadow-inner min-h-[200px]">
                 <iframe width="100%" height="100%" frameBorder="0" title="Location Map" scrolling="no" src="https://maps.google.com/maps?q=서울특별시+서초구+주흥길+12&hl=ko&z=16&output=embed" className="w-full h-full opacity-90 transition-all duration-500 grayscale-[20%] hover:grayscale-0" />
              </div>
              <div className="w-full shrink-0">
                  <button 
          ...
          <div className={`col-start-1 row-start-1 flex flex-col h-full w-full transition-all duration-500 ease-in-out ${showParking ? 'opacity-100 translate-x-0 z-10' : 'opacity-0 translate-x-10 pointer-events-none'}`}>
              <div className="flex items-center justify-between mb-6 shrink-0">
                  <h3 className="text-xl font-bold text-[#1A1A1A]">공영 주차장 안내</h3>
                  <button 
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4">
                   ...
                   <div className="bg-white/60 border border-black/5 rounded-2xl p-5">
                       <h4 className="font-bold text-[#1A1A1A] text-lg mb-1">1. 언구비 공영 주차장</h4>
                       <p className="text-xs text-[#C85E53] font-bold mb-3">공연장과 20M 거리</p>
                       <div className="space-y-1 text-sm text-[#1A1A1A]/80">
                          <p>강남대로83길 55</p>
                          <div className="flex gap-3 font-medium text-gray-500 mt-2 text-xs">
                   ...
                   {/* Parking Item 2 */}
                   <div className="bg-white/60 border border-black/5 rounded-2xl p-5">
                       <h4 className="font-bold text-[#1A1A1A] text-lg mb-1">2. 방음언덕형 공영 주차장</h4>
              ...
              <div className="mt-6 w-full shrink-0">
                   <button onClick={() => setShowParking(false)} className="w-full py-4 bg-[#1A1A1A] text-white text-sm font-bold rounded-xl hover:bg-[#1A1A1A]/90 transition-all shadow-lg active:scale-[0.98]">
                      돌아가기
                   </button>
              </div>
          </div>
        ...
        <div id="members-card" className="order-3 group relative bg-white/60 backdrop-blur-md border border-black/5 rounded-[40px] p-10 overflow-hidden hover:border-black/20 transition-all duration-500 min-h-[500px] flex flex-col shadow-xl">
          <div className="relative z-10 mb-8">
            <h3 className="text-xl font-bold text-[#1A1A1A] mb-6">밴드 구성원</h3>
            <div className="space-y-2 text-sm text-[#1A1A1A]/80 font-medium leading-relaxed">
          ...
          <div className="flex-1 flex items-center justify-center select-none relative w-full">
              <div className="relative w-full h-full max-h-[280px] flex items-center justify-center">
                  ...
                  />
                  {/* Shadow */}
                  <div className="absolute -bottom-2 w-[60%] h-6 bg-black/15 blur-xl rounded-[100%] z-0"></div>
              </div>
          </div>
        </div>

        {/* Playlist Card - UPDATED COLOR & ANIMATION */}
        <div id="playlist-card" className="order-4 group relative bg-white/60 backdrop-blur-md border border-black/5 rounded-[40px] p-10 overflow-hidden hover:border-black/20 transition-all duration-500 min-h-[500px] flex flex-col shadow-xl">
          <div className="relative z-10 w-full mb-10">
            <h3 className="text-xl font-bold text-[#1A1A1A] mb-8">플레이 리스트</h3>
            <div className="flex flex-col gap-8 text-sm text-[#1A1A1A]/80 font-medium leading-relaxed">
              <div>
                <span className="text-[#1A1A1A] font-black text-lg block mb-2">SET.1</span>
                <p>등대</p>
                <p>FIND ME!</p>
              </div>
              <div>
                <span className="text-[#1A1A1A] font-black text-lg block mb-2">SET.2</span>
              ...
              <div>
                <span className="text-[#1A1A1A] font-black text-lg block mb-2">SET.3</span>
                <p>시퍼런 봄</p>
                <p>꽃에 망령</p>
                <p>계속 웃을 순 없어!</p>
                <p>숙명</p>
              ...
              <div>
                <span className="text-[#1A1A1A] font-black text-lg block mb-2">SET.4</span>
                <p>불꽃놀이</p>
                <p>ㅈㅣㅂ</p>
                <p>항해</p>
                <p>Drowning</p>
